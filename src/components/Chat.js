...
function Chat({ setBlurBackground }) {
    ...
    useEffect(() => {
        ...
        if (newChat) {
            ...
            socket.emit(
                "join", { userid, clickedUserList, randomRoomNumber },
                ({ error }) => {}
            );
        } else {
    ...
    return ( <
        div > {
            message && emojiPicker && chosenEmoji && ( <
                >
                <
                div className = "emojiPicker-background"
                onClick = { emojiCloser } > < /div> <
                />
            )
        } <
...

        {
            chatRoom ? .map((user) => ( <
                ...
                <
                div key = { user.id }
                className = "my-message-chat-head-id" >
                <
                span key = { user.id } > { user.name } < /span> <
                /div> <
                />
            ...
            messages.map((item) => ( <
                > {
                    item.id === loginUserInfo ? .id ? ( <
                        ...
                        div >
                        <
                        div className = "my-message-chat-message" > { loginUserInfo.name } <
                        /div>
                        ...
                        div className = "my-message-chat-message-text" > {
                            ...
                            "data:image/jpeg;base64" ? ( <
                                ...
                                div className = "my-message-chat-message-text-container" > { item.text } <
                                /div>
                            )
                        } <
                        /div> <
        ...
        <
        BsEmojiSmile className = "emoji-picker"
